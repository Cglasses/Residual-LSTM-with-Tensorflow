...
class LSTM(object):
	...
	def build_lstm_cell(self, last_output, last_memmory, input_tensor, cell_code):
		...
		s2, s2_vars = self.inside_LSTM_nn(10, 10, self.nn1_inside_LSTM_outputfeature_num, '1', cell_code)
		s2 = tf.nn.sigmoid(s2, name = 's2_sig_' + cell_code)

		s3, s3_vars = self.inside_LSTM_nn(10, 10, self.nn2_inside_LSTM_outputfeature_num, '2', cell_code)
		s3 = tf.nn.sigmoid(s3, name = 's3_sig_' + cell_code)

		s4, s4_vars = self.inside_LSTM_nn(10, 10, self.nn3_inside_LSTM_outputfeature_num, '3', cell_code)
		s4 = tf.tanh(s4, name='tanh_s4_' + cell_code)

		s5, s5_vars = self.inside_LSTM_nn(10, 10, self.nn4_inside_LSTM_outputfeature_num, '4', cell_code)
...
class ResidualLSTM(LSTM):
	def __init__(self, batch_size, num_lstm_cell, input_feature_num, output_feature_num, memory_feature_num, residual_season=7, dropout_val=0.95):
		"""
		A Constructor
		Args:
			batch_size          :       an integer, the size of batch
		...
		"""
		super(ResidualLSTM, self).__init__(input_feature_num, output_feature_num, memory_feature_num, dropout_val=0.95)
		self.batch_size = batch_size
		self.num_lstm_cell = num_lstm_cell
	...
	def build_net(self):
		...
		for i in range(self.num_lstm_cell):
			...
			memmories[i] = memory
			if i >= 7:
				last_memmory = 0.7 * memory + 0.3 * memmories[i-7]
			else:
				last_memmory = memory 
			outs.append(out)
			cell_vars.extend(cell_var)
